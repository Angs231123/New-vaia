// Single-file Pages Worker so the whole site (static pages + admin login +
// performers/roster API) can be deployed via Cloudflare's dashboard
// drag-and-drop, which doesn't support a `functions/` folder but does
// support a top-level `_worker.js`. This is the same logic that used to
// live in functions/, just combined into one file with no imports.
